...
print(startPopulation_Lion,startPopulation,infant_mortality,Lioninfant_mortality,youth_mortality,agriculture,disaster_chance,min_age_fertility,max_age_fertility,food,mutation,rain)
...
show_gender_button_ax = plt.axes([0.60, 0.01, 0.05, 0.05])
gender_button = Button(show_gender_button_ax, 'gender', hovercolor='0.9')
...
def add_people(event):
    peopleDictionary.append(Person(15, random.uniform(0, 20), random.uniform(0, 20), random.uniform(1, 2), 1.5,random.uniform(1,3),parent="?",immune=random.randint(0,10)))
...
def show_gender(event):
    global show_gender_var
    show_gender_var=not(show_gender_var)
...
gender_button.on_clicked(show_gender)
# Create animation
animation = FuncAnimation(fig, update, frames=2000, interval=100, repeat=False, blit=False)
# Add legend
ax.legend()
